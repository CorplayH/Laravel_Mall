    ...
    <div class="card">
        <div class="card-header">
            <ul class="nav" style="border-bottom: 1px solid #0b0c0f">
                ...
                <li class="nav-item">
                    <a href="{{route('category.category.edit',$category)}}" class="nav-link active">修改分类</a>
                </li>
            </ul>
        </div>
                    ...
                    <div class="col-md-10">
                        <select name="pid" class="select2 form-control" id="basicSelect">
                            ...
                            @foreach($parentCategories as $v)
                                <option value="{{$v['id']}}"
                                        level="{{$v['_level']}}" {{$v['id'] == $category['pid']? 'selected' :'' }}>{{$v['_cname']}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                        ...
                        <div class="mb-2">
                            @foreach($attributes as $k => $attr)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="attrs[]"
                                           id="attr{{$attr['id']}}" value="{{$attr['id']}}"
                                    {{in_array($attr['id'],$availableAttr->toArray()) ? 'checked' : ''}}>
