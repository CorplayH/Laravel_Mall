...
@section('content')
    <div class="card">
        <div class="card-header">
            <ul class="nav">
                <li class="nav-item">
                    <a href="" class="nav-link">商品列表</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link">添加商品</a>
                </li>
                <li class="nav-item">
                    <a href="" class="nav-link active">修改商品</a>
                </li>
            </ul>
        </div>
        <form action="{{route('goods.goods.update',$good)}}" method="post">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-3">
                        <select name="category_id[]" class=" select2 form-control" id="topCategory">
                            ...
                            @foreach($topCategory as $category)
                                <option
                                    value="{{$category['id']}}" {{in_array($category['id'],$good['category_id']) ? 'selected' : ''}}>{{$category['cname']}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select name="category_id[]" class="form-control" id="secondCategory">
                            ...
                            @foreach($secondCategory as $v)
                                <option
                                    value="{{$v['id']}}" {{$good['category_id'][1] == $v['id'] ? 'selected' : ''}}>{{$v['cname']}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select name="category_id[]" class="form-control" id="thirdCategory">
                            ...
                            @foreach($thirdCategory as $v)
                                <option
                                    value="{{$v['id']}}" {{$good['category_id'][2] == $v['id'] ? 'selected' : ''}}>{{$v['cname']}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-10" id="attrs">
                        @foreach($attrs as $attr)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="checkbox" name="attrs[]" id="attr{{$attr['id']}}"
                                       {{in_array($attr['id'],$availableAttr) ? 'checked' : ''}} value="{{$attr['id']}}">
                                <label class="form-check-label" for="attr{{$attr['id']}}">{{$attr['aname']}}</label>
                            </div>
                        @endforeach
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <button id="goodsPics" class="btn btn-bg-gradient-x-purple-blue mb-2" type="button">
                            选择图片
                        </button>
                        <div id="box">
                            @foreach($good['images'] as $v)
                            <img src="{{$v}}" alt="">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <div id="editormd">
                            <textarea style="display:none;" name="description">{{$good['description']}}</textarea>
                        </div>
                    </div>
                </div>
                <div class="form-group row" id="attrs">
                    ...
                    <div class="col-md-10" id="attrsss">
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" name="is_recommend"
                                   {{$good['is_recommend'] ? 'checked' : ''}} id="is_recommend" value="1">
                            <label class="form-check-label" for="is_recommend">是</label>
                        </div>
                    </div>
                </div>
            </div>
